...
describe("DELETE /users", () => {
  ...
  beforeEach(async () => {
    ...
    userNotAdmin = await userRepo.save(destroyUserRouteMock.userNotAdminTemplate);
  ...
  it("Error: Must not be able to destroy - Invalid signature", async () => {
    const response = await supertest(app).delete(destroyAdminUrl).set("Authorization", `Bearer ${tokenMock.invalidSignature}`);
  ...
  it("Error: Must not be able to destroy - JWT malformed", async () => {
    const response = await supertest(app).delete(destroyAdminUrl).set("Authorization", `Bearer ${tokenMock.jwtMalformed}`);
